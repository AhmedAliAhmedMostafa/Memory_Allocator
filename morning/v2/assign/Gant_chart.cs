...
namespace Assign
{
    public partial class Gant_chart : Form
    {
        ...
        {
            public int process_id, start, size,HoleThatContainsIt;
            public string name;

            public segment(int pid, int start, int size, string name,  int t )
            {
                this.process_id = pid;
                this.start = start;
                this.size = size;
                this.name = name;
                this.HoleThatContainsIt = t; 
            ...
            for (int i = 0; i < Form1.segment_size.Count(); i++)
            {
                ...
                foreach (KeyValuePair<int, Form1.hole> p in Form1.sortedHoles_in_address)
                {
                    ...
                    if (Form1.segment_size[i] <= p.Value.size)
                    {
                        ...
                        segment s = new segment(allocated_process_id, p.Key, Form1.segment_size[i], Form1.segment_name[i] ,p.Value.id);
        ...
        public void Allocate_best_fit()
        {
            ...
            for (int i = 0; i < Form1.segment_size.Count(); i++)
            {
                ...
                for (int j = 0; j < Form1.holes_start.Count(); j++)
                {
                    ...
                    if (Form1.segment_size[i] <= Form1.holes_size[j])
                    {

                        segment s = new segment(allocated_process_id, Form1.holes_start[j], Form1.segment_size[i], Form1.segment_name[i], Form1.sortedHoles_in_address[Form1.holes_start[j]].id);
                        ...
                        Form1.holes_size[j] -= Form1.segment_size[i];


                        if (Form1.holes_size[j] == 0)
                        {
                        ...
                        {
                            Form1.holes_start[j] += Form1.segment_size[i];
        ...
        public void sort_holes_size()
        {
            ...
            for (int i = 0; i < Form1.holes_num; i++)
            {
                for (int j = i + 1; j < Form1.holes_num; j++)
                {
                    if (Form1.holes_size[j] < Form1.holes_size[i])
                    {
                        holder1 = Form1.holes_start[j];
                        holder2 = Form1.holes_size[j];
                        Form1.holes_start[j] = Form1.holes_start[i];
