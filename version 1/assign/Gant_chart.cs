    ...
    public partial class Gant_chart : Form
    {
        ...
        public void Allocate_best_fit()
        {
            ...
            for (int i = 0; i < Form1.segment_size.Count(); i++)
            {
                ...
                foreach (KeyValuePair<int, Form1.hole> p in Form1.sortedHoles_inSize)
                {
        ...
        public void combact()
        {
            ...
            {
                int end_address = Form1.holes_start[i] + Form1.holes_size[i];
                for (int j = 0; j < Form1.holes_num; j++)
                {
                    if(Form1.holes_start[j]==end_address)
                    {
                        end_address += Form1.holes_size[j];
